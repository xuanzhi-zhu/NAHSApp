...
%%%scale the parameter adaptation parameters 

scale_k=4;%-4,-3,-2,-1,0,1,2,3,4
...
delta_bv=200*scale_k^(-1);%the larger the smaller overshoot
delta_bo=2000*scale_k^(-1);
...
parameters.gammav=15*scale_k;%25
parameters.gammao=1500*scale_k;%10000
...
% %numerically estimate the upper bound of pd3
% % Simulation horizon
% TSPAN = [0 600];
% JSPAN = [0 10*TSPAN(2)];
% rule=2;%2 for flow                                                                      
...
% [t,j,psi]=HyEQsolver(@F_ref,@G_ref,@C_ref,@D_ref,psi0,TSPAN,JSPAN,rule,options);
% ref_vec=zeros(length(t),8);
% for i=1:1:length(t)
%     [~,ref_] = ref(psi(i),t(i),parameters);
%     ref_vec(i,:)=ref_';
% end
% % pd0_vec=ref_vec(:,1:2);
% % pd1_vec=ref_vec(:,3:4);
...
% pd3_vec=ref_vec(:,7:8);
% 
% % pd1_ub=max(sum(pd1_vec.^2,2))^(1/2);%=v_max
% pd3_ub=max(sum(pd3_vec.^2,2))^(1/2);
