function dhbo = chio(xi,ref,parameters)
e1=parameters.e1;
e2=parameters.e2;
S=parameters.S;
% m=parameters.m;
...
% dhbo=-inv(h3).*theta2.*hbo + inv(h3).*invJ.*e2'*phi*zo;

bar_bo=parameters.bar_bo;
bar_hbo=parameters.bar_hbo;
delta_bo=parameters.delta_bo;

muo=ho.*invJ.*e2'*phi*zo;
dhbo=inv(gammao).*proj(muo,hbo, bar_bo,bar_hbo,delta_bo);
